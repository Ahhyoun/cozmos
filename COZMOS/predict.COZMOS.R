### I revised 'pred.rpart.R' in rpart package.
...
#     type <- match.arg(type)
	...
	where <- pred.for.newdata (object, rpart.matrix(newdata) )     
	}
    frame <- object$frame
...
# 	rownames(frame$yval2)<- rownames(frame)
# 	pred <- frame$yval2[as.character(where), 1L + nclass + 1L:nclass, drop = FALSE]
